...
export default function ResumeMatcher() {
  ...
  // Multi-dimensional Result State
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Execution failure: ${response.statusText}`);
      }

      const data = await response.json();

      // Ensure data has reasonable fallbacks for empty AI results
      const safeData = {
        ...data,
    ...
    <div className="min-h-screen bg-[#030712] text-gray-100 p-4 md:p-8 relative overflow-x-hidden selection:bg-indigo-500/30">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 mb-16">
          <div className="flex items-center gap-6">
            <div className="relative group p-1">
              <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl blur-lg opacity-40 group-hover:opacity-100 transition duration-1000"></div>
            ...
            <div>
              ...
              <p className="text-gray-400 mt-2 font-medium flex items-center gap-2">
                ...
                AI Career Intelligence Engine v2.0
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="px-6 py-3 glass-card rounded-2xl flex items-center gap-3 border-emerald-500/20 bg-emerald-500/5">
              <div className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse"></div>
              <span className="text-xs font-black uppercase tracking-widest text-emerald-400">System Nominal</span>
            </div>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          {/* Analysis View (Right Panel) */}
          <div className="lg:col-span-4">
            <div className="glass-card rounded-[2.5rem] p-8 border-white/5 bg-white/5 h-full flex flex-col justify-center">
              {loading ? (
                <div className="flex flex-col items-center text-center animate-pulse">
                  <div className="w-32 h-32 border-4 border-white/5 border-t-blue-500 rounded-full animate-spin mb-6"></div>
                ...
                <div className="flex flex-col items-center animate-scale-in">
                  <CircularProgress
                    ...
                    label="Overall Match Reliability"
                    size={200}
                    strokeWidth={12}
                  ...
                  <div className="mt-8 p-4 bg-emerald-500/5 rounded-2xl border border-emerald-500/20 flex items-center gap-3 w-full">
                    <Shield className="text-emerald-400" size={16} />
                    <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">Scan Complete • Accuracy: 99.4%</span>
                  </div>
                </div>
              ) : (
                            ...
                            <div key={i} className="p-5 glass-card rounded-2xl bg-white/[0.02] border-white/5 hover:bg-white/[0.04] transition-all group">
                              ...
                              <div className="space-y-3">
                                <div className="flex items-center justify-between text-[10px] text-gray-400 font-medium">
                                  <span>Learning Curve: {gap.learning_curve || 'Medium'}</span>
                                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                      <div>
                        <h5 className="text-white font-bold mb-4 text-sm flex items-center gap-2">
                          <div className="h-1.5 w-1.5 rounded-full bg-blue-500"></div> Vector Alignment Strength
                        </h5>
                        <div className="space-y-4">
                          <div className="h-4 bg-white/5 rounded-full overflow-hidden flex">
                            <div className="h-full bg-blue-500 w-[70%]"></div>
                            <div className="h-full bg-indigo-500 w-[15%] opacity-50"></div>
                          </div>
                          <p className="text-[10px] text-gray-400 leading-relaxed font-medium">The resume's latent vector representation shows high cosine similarity with the {selectedRole === "Custom" ? "job requirements" : selectedRole + " ontology"}.</p>
                        </div>
                      </div>
                      <div>
                        <h5 className="text-white font-bold mb-4 text-sm flex items-center gap-2">
                          <div className="h-1.5 w-1.5 rounded-full bg-purple-500"></div> Role Displacement Metrics
                        </h5>
                        <div className="space-y-4">
                          <div className="h-4 bg-white/5 rounded-full overflow-hidden flex">
        ...
        <footer className="pt-20 pb-12 flex flex-col items-center gap-6 opacity-30">
          ...
          <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest text-center">
            Professional AI Feedback • Industry Standard Analysis • Pattern Matching
          </p>
        </footer>
      </div>
